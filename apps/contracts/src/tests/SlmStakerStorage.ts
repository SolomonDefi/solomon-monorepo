...
describe('SLM Staker Storage', function () {
    ...
    it('Outstanding votes', async () => {
        ...
        chai.expect(await storage.getOutstandingVotes(account1.address)).to.equal(expectedOutstanding)
    ...
    it('Dispute vote count', async () => {
        chai.expect(await storage.getDisputeVoteCount(account1.address, chargeback.address)).to.equal(0)
        ...
        let expectedOutstanding = 0 + increase
        chai.expect(await storage.getDisputeVoteCount(account1.address, chargeback.address)).to.equal(expectedOutstanding)
    ...
    it('Vote history', async () => {
        ...
        chai.expect(await storage.getVoteHistoryCount(account1.address)).to.equal(expectedHistory)
    ...
    it('Unstaked info', async () => {
        ...
        chai.expect(await storage.getUnstakedAmount(account1.address, unstakeIndex)).to.equal(unstakeAmount)
        chai.expect(await storage.getUnstakedTime(account1.address, unstakeIndex)).to.equal(currentTime)
        chai.expect(await storage.getUnstakeCount(account1.address)).to.equal(unstakeIndex + 1)
        ...
        chai.expect(await storage.getUnstakedAmount(account1.address, unstakeIndex)).to.equal(newUnstakeAmount)
        chai.expect(await storage.getUnstakedTime(account1.address, unstakeIndex)).to.equal(newTime)
        chai.expect(await storage.getUnstakeCount(account1.address)).to.equal(unstakeIndex + 1)

        // Remove a non-existent unstake entry
        await chai.expect(storage.deleteUnstakedInfo(account1.address, 1)).to.be.revertedWith('reverted with panic code 0x32 (Array accessed at an out-of-bounds or negative index)')
        // Remove an exiting unstake entry
        await storage.deleteUnstakedInfo(account1.address, 0)
        chai.expect(await storage.getUnstakedAmount(account1.address, unstakeIndex)).to.equal(0)
        ...
        chai.expect(await storage.getUnstakedSLM(account1.address)).to.equal(unstakeAmount + newUnstakeAmount)
    })
    ...
    it('Sets last reward index and withdrawal times', async () => {
        ...
        chai.expect(await storage.getLastWithdrawalTime(account1.address)).to.equal(currentTime)
    })
})