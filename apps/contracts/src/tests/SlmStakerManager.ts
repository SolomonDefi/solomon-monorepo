...
describe('SLM Staker Manager', function () {
    ...
    it('Staking and unstaking', async () => {
        ...
        let timeFlag = false;
        if(Math.abs(await manager.getUnstakedTime(account1.address, 0) - currentTime) <= 10) {
            timeFlag = true;
        }
    ...
    it('Handles rewards', async () => {
        ...
        chai.expect(await manager.getRewardAmountHistory(0)).to.equal((rewardPercent1 * storageBalance) / 100)
